import { useMemo, useState } from 'react'
import { Box, Typography, useTheme } from '@mui/material'
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts'
import { SectionCard } from '../common/SectionCard'
import { RangeSelect } from './RangeSelect'
import { chartPalette } from '../../theme/theme'
import { formatDateShort, formatDuration } from '../../utils/format'
import type { TrendPoint } from '../../types/models'

const RANGE_OPTIONS = [
  { value: '10', label: 'Last 10 Runs' },
  { value: '20', label: 'Last 20 Runs' },
  { value: '50', label: 'Last 50 Runs' },
]

function DurationTooltip({ active, payload, label }: any) {
  const theme = useTheme()
  if (!active || !payload?.length) return null
  const row = payload[0]?.payload

  return (
    <Box
      sx={{
        bgcolor: theme.customTokens.cardBackground,
        border: `1px solid ${theme.palette.divider}`,
        borderRadius: 1.5,
        px: 1.5,
        py: 1,
      }}
    >
      <Typography variant="caption" sx={{ fontWeight: 700, display: 'block' }}>
        Execution #{row?.executionId}
      </Typography>
      <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
        {formatDateShort(label)}
      </Typography>
      <Typography variant="body2" sx={{ fontWeight: 700, color: chartPalette.duration }}>
        {formatDuration(row?.duration ?? 0)}
      </Typography>
    </Box>
  )
}

export function DurationTrendChart({ data }: { data: TrendPoint[] }) {
  const theme = useTheme()
  const [range, setRange] = useState('20')

  const sliced = useMemo(() => data.slice(-Number(range)), [data, range])

  return (
    <SectionCard
      title="Duration Trend"
      actions={<RangeSelect value={range} onChange={setRange} options={RANGE_OPTIONS} />}
    >
      <Box sx={{ width: '100%', height: 300 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={sliced} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
            <defs>
              <linearGradient id="durationGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={chartPalette.duration} stopOpacity={0.4} />
                <stop offset="95%" stopColor={chartPalette.duration} stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid stroke={theme.customTokens.chartGrid} vertical={false} />
            <XAxis
              dataKey="date"
              tickFormatter={formatDateShort}
              tick={{ fontSize: 11, fill: theme.palette.text.secondary }}
              axisLine={{ stroke: theme.palette.divider }}
              tickLine={false}
            />
            <YAxis
              tickFormatter={(v) => formatDuration(v).replace(' ', '')}
              tick={{ fontSize: 11, fill: theme.palette.text.secondary }}
              axisLine={false}
              tickLine={false}
              width={44}
            />
            <Tooltip content={<DurationTooltip />} />
            <Area
              type="monotone"
              dataKey="duration"
              stroke={chartPalette.duration}
              fill="url(#durationGradient)"
              strokeWidth={2}
              dot={false}
            />
          </AreaChart>
        </ResponsiveContainer>
      </Box>
    </SectionCard>
  )
}
