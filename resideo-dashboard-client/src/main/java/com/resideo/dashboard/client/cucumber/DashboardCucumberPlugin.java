package com.resideo.dashboard.client.cucumber;

import com.resideo.dashboard.client.DashboardReporter;
 ...
 *
 * Activate via:
 *   cucumber.properties:  cucumber.plugin=com.resideo.dashboard.client.cucumber.DashboardCucumberPlugin
 *
 * Configuration (system properties or environment variables):
 *   RESIDEO_DASHBOARD_URL   or  resideo.dashboard.url   = http://localhost:8080  (default)
 *   RESIDEO_API_TOKEN       or  resideo.api.token       = rd_xxx... (optional)
 *   RESIDEO_EXECUTION_ID    or  resideo.execution.id    = pre-created execution ID (optional)
 *   RESIDEO_PROJECT_ID      or  resideo.project.id      = project UUID (optional)
 */
public class DashboardCucumberPlugin implements ConcurrentEventListener {
    ...
    public DashboardCucumberPlugin() {
        this.reporter = new DashboardReporter(getEnv("RESIDEO_DASHBOARD_URL", "resideo.dashboard.url", DEFAULT_URL),
                                              getEnv("RESIDEO_API_TOKEN", "resideo.api.token", null));
        this.executionId = getEnv("RESIDEO_EXECUTION_ID", "resideo.execution.id", null);
        this.projectId = getEnv("RESIDEO_PROJECT_ID", "resideo.project.id", null);
    ...
    private void log(String msg) {
        System.out.println("[ResideoDashboard] " + msg);
    }
