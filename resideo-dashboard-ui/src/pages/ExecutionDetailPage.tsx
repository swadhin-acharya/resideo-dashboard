...
import {
  ...
  TableContainer, TableHead, TableRow, Paper, IconButton, TextField, Button,
  Collapse, Tooltip,
} from '@mui/material';
...
import { getExecution, updateName, cancelExecution } from '../api/executions';
import api from '../api/client';
import { Execution, WsMessage } from '../types/execution';
...
export default function ExecutionDetailPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/executions/${id}/logs`).then(r => {
      const logs: LogEntry[] = r.data;
      if (logs.length > 0) {
        setLogLines(logs.map(l => l.message));
      }
      setLogsLoaded(true);
  ...
  const handleWsMessage = useCallback((msg: WsMessage) => {
    if (msg.executionId !== id) return;
    switch (msg.type) {
      case 'EXECUTION_LOG':
        setLogLines(prev => [...prev.slice(-200), msg.data.message as string]);
        break;
  ...
  useWebSocket(handleWsMessage, id);
  ...
  const handleSaveName = async () => {
    if (id && newName.trim()) {
      ...
      setExec({ ...exec, name: newName.trim() });
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          {editingName ? (
            <>
              <TextField size="small" value={newName} autoFocus
                onChange={(e) => setNewName(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleSaveName()}
                sx={{ minWidth: 250 }} />
              <IconButton size="small" color="primary" onClick={handleSaveName}><CheckIcon /></IconButton>
              <IconButton size="small" onClick={() => setEditingName(false)}><CloseIcon /></IconButton>
            </>
          ) : (
            <>
              <Typography variant="h4">{exec.name || exec.buildNumber || 'Execution Detail'}</Typography>
              <IconButton size="small" onClick={() => { setNewName(exec.name || ''); setEditingName(true); }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          {isRunning && (
            <Button variant="outlined" color="error" size="small"
              startIcon={<CloseIcon />} onClick={() => id && cancelExecution(id)}>
              Cancel
            </Button>
          )}
            ...
            <CardContent>
              <Grid container spacing={2}>
                <Grid size={{ xs: 6, md: 3 }}>
                  ...
                  <Typography fontWeight={600}>{exec.buildNumber || '-'}</Typography>
                </Grid>
                <Grid size={{ xs: 6, md: 3 }}>
                ...
                <Grid size={{ xs: 6, md: 3 }}>
                  ...
                  <Typography fontWeight={600}>{exec.environment || '-'}</Typography>
                </Grid>
                <Grid size={{ xs: 6, md: 3 }}>
                  <Typography variant="body2" color="text.secondary">Firmware</Typography>
                  <Typography fontWeight={600}>{exec.firmwareVersion || '-'}</Typography>
                </Grid>
                <Grid size={{ xs: 6, md: 3 }}>
                ...
                <Grid size={{ xs: 6, md: 3 }}>
                  ...
                  <Typography fontWeight={600}>{exec.appVersion || '-'}</Typography>
                </Grid>
                <Grid size={{ xs: 6, md: 3 }}>
                  <Typography variant="body2" color="text.secondary">Triggered By</Typography>
                  <Typography fontWeight={600}>{exec.triggeredBy || '-'}</Typography>
                </Grid>
                <Grid size={{ xs: 6, md: 3 }}>
                          ...
                          <Collapse in={expandedScenario === s.id}>
                            <Box sx={{ p: 2, bgcolor: 'action.hover' }}>
                              {loadingSteps[s.id] ? (
                                  ...
                                  <TableBody>
                                    {scenarioSteps[s.id].map((step, i) => (
                                      <TableRow key={step.id || i}>
                                        <TableCell>
                                          <Typography variant="body2" sx={{ fontSize: '0.8rem' }}>{step.stepName}</Typography>
                                          {step.logText && (
                                          ...
                                          {step.screenshotPath && (
                                            <Box sx={{ mt: 1 }}>
                                              <img src={step.screenshotPath} alt="Step screenshot"
                                                style={{ maxWidth: 300, maxHeight: 200, borderRadius: 4, border: '1px solid #333' }}
                                                onClick={() => window.open(step.screenshotPath, '_blank')} />
                                            </Box>
                                          )}
                                        </TableCell>
                                        <TableCell>
                                ...
                                <Typography variant="body2" color="text.secondary">Steps will appear as the scenario executes...</Typography>
                              ) : (
      ...
      {exec.scenarios && exec.scenarios.some(s => s.deviceName) && (
        <Card sx={{ mb: 3 }}>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 2 }}>Devices Used</Typography>
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              {Array.from(new Set(exec.scenarios.filter(s => s.deviceName).map(s => s.deviceName))).map((d, i) => (
                <Chip key={i} label={d} variant="outlined" color="primary" />
              ))}
            </Box>
          </CardContent>
        </Card>
      ...
      {exec.reportPath && (
        <Card sx={{ mb: 3 }}>
          <CardContent>
            ...
            <Button variant="outlined" component="a" href={`/api/v1/executions/${exec.id}/report`} target="_blank">
              View Full Report
            </Button>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>
            Execution Log
            {isRunning && <Chip label="LIVE" size="small" color="primary" sx={{ ml: 1 }} />}
          </Typography>
          <Box
            sx={{
              bgcolor: '#1e1e1e',
