...
/**
 * Cucumber plugin that publishes execution events to the OpenQA Dashboard in real time.
 *
 * Activate via:
 ...
 *   OPENQA_DASHBOARD_URL   or  openqa.dashboard.url   = http://localhost:8080  (default)
 *   OPENQA_API_TOKEN       or  openqa.api.token       = rd_xxx... (optional)
 *   OPENQA_EXECUTION_ID    or  openqa.execution.id    = pre-created execution ID (optional)
 *   OPENQA_PROJECT_ID      or  openqa.project.id      = project UUID (optional)
 */
public class DashboardCucumberPlugin implements ConcurrentEventListener {
    ...
    public DashboardCucumberPlugin() {
        this.reporter = new DashboardReporter(getEnv("OPENQA_DASHBOARD_URL", "openqa.dashboard.url", DEFAULT_URL),
                                              getEnv("OPENQA_API_TOKEN", "openqa.api.token", null));
        this.executionId = getEnv("OPENQA_EXECUTION_ID", "openqa.execution.id", null);
        this.projectId = getEnv("OPENQA_PROJECT_ID", "openqa.project.id", null);
